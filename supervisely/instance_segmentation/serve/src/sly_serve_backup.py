...
def preprocess():
    ...
    model_local_path = os.path.join(my_app.data_dir, 'model.pth')
    config_local_path = os.path.join(my_app.data_dir, 'model_config.yaml')

    if modelWeightsOptions == "pretrained":
        sly.fs.download(curr_model_url, config_local_path, my_app.cache, progress)

    elif modelWeightsOptions == "custom":
        file_info = my_app.public_api.file.get_info_by_path(TEAM_ID, custom_weights)

        dir_of_custom_weights = os.path.dirname(custom_weights)
    ...
    cfg.merge_from_file(model_zoo.get_config_file(config_local_path))

    cfg.MODEL.WEIGHTS = model_local_path
