...
def create_splits(api: sly.Api, task_id, context, state, app_logger):
    ...
    try:
        ...
        fields = [
            ...
            {"field": "data.trainImagesCount", "payload": None if train_set is None else len(train_set)},
            {"field": "data.valImagesCount", "payload": None if val_set is None else len(val_set)},
        ]
